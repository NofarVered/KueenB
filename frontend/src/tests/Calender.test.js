...
test('checking calendar', () => {
   ...
   const {getByText} = renderWithRouter(<Calendar name={'tom'} email={'t@g.c'} mapRegistersByDay={testDB}/>, { route })

   //
   fireEvent.click(getByText(getClosestSundayOfNextWeek()))

   expect(getByText(getClosestSundayOfNextWeek()).parentElement).toHaveClass("selected");
})