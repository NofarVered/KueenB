...
class UserCalendar extends React.Component {
  ...
  onContinueClick = () => {
    if (
      this.props.mapRegistersByDay[this.state.selectedDate] &&
      ...
      alert(
        "You have successfully signed for " +
          format(this.state.selectedDate, "dd/MM/yyyy")
      );
      console.log(
        this.props.email,
  ...
  render() {
    const dicValue = this.props.mapRegistersByDay[
      format(this.state.selectedDate, "dd/MM/yyyy")
    ];
    const maxPeople = 20;
    const numOfRegistersString = dicValue
      ? ` ${dicValue.length} registered (${
          maxPeople - dicValue.length
        } available)`
        ...
        <div className="box">
          <div className="dayWindow">
            <p className="dateHeadline">
              {format(this.state.selectedDate, "EEEE")},{" "}
              {format(this.state.selectedDate, "dd.MM")}
            </p>
            <div className="numOfRegisters">{numOfRegistersString}</div>
            <Link to="/registers">
