...
async function createEmployee(employeeDetails) {
  try {
    ...
    await pool.query(
      `INSERT INTO employees (email, name, HS, arrivalDate) VALUES ($1, $2, $3, $4)`,
      [
...
async function updateEmployee(employeeDetails) {
  try {
    ...
    await pool.query(
      ...
      [employeeDetails.email, employeeDetails.arrivalDate]
    );
    const results = await pool.query("SELECT * FROM employees WHERE email=$1", [
      employeeDetails.email,
    ]);
